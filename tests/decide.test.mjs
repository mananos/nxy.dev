...
const REWRITE_CASES = [
  ...
  ['npm test | tail -20', 'rtk npm run test', true],
  ['mvn test 2>&1 | tail -n 150', 'rtk mvn test 2>&1', true],
  ['cd "C:\\x\\api" && export JAVA_HOME="C:\\j" && ./mvnw.cmd -B test 2>&1 | tail -n 150', 'rtk cd "C:\\x\\api" && export JAVA_HOME="C:\\j" && mvnw.cmd -B test 2>&1', true],
