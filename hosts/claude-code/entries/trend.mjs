...
import { dailyFromGain, readLedgerRows, sessionSavings } from '../../../core/filter/ledger.mjs';
import { engineInfo } from '../../../core/filter/engine.mjs';
import { claudeSettingsFiles } from '../settings.mjs';
import { rtkGain } from '../../../core/filter/rtk.mjs';
import { sumTtlWhatIf } from '../../../core/metrics/cache-ttl.mjs';
...
function filterRowsFor(sessionCwd, sessionId) {
  ...
  if (!filterCache.has(dir)) {
    const p = join(nxyRuntimeDir(dir), 'metrics', 'filter.jsonl');
...
const total = sumUsd(rows);
const rebuilds = summarizeBreaks(sessions.flatMap((x) => x.cacheBreaks));
const ttlWhatIf = sumTtlWhatIf(sessions.map((x) => x.ttlWhatIf));
if (opts.json) {
  console.log(JSON.stringify({ since, by, sessions, periods: rows, total, rebuilds, ttlWhatIf }, null, 2));
  process.exit(0);
}
