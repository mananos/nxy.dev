...
const filterPath = join(nxyRuntimeDir(s.cwd || cwd), 'metrics', 'filter.jsonl');
...
if (s.cacheBreaks.length) {
  ...
  for (const l of formatBreaks(summarizeBreaks(s.cacheBreaks), s.usage.usd || 0, fmtTokens)) out.push(`  ${l}`);
  for (const cb of s.cacheBreaks.slice(0, 5)) out.push(`  ${fmtDate(cb.ts)} · ${cb.cause} · ${fmtTokens(cb.uncached)} uncached of ${fmtTokens(cb.total)} · ttl ${cb.ttl}${cb.agentType ? ` · agent ${cb.agentType}` : ''}`);
} else {
  out.push('CACHE BREAKS: none');
}
const ttlLine = formatTtlWhatIf(s.ttlWhatIf);
if (ttlLine) out.push(`  ${ttlLine}`);
