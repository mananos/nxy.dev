...
/** @typedef {import('../../core/metrics/aggregate.mjs').Call} Call */
...
export function parseFile(path, opts = {}) {
  ...
  // What happened between two API calls, so a cache rebuild can be attributed: a subagent the
  // thread waited for, a compaction, or (from the gap) plain idle time.
  /** @type {'subagent'|'compact'|null} */
  ...
  for (const line of text.split('\n')) {
    ...
    if ((e.type === 'system' && e.subtype === 'compact_boundary') || e.isCompactSummary) pendingCause = 'compact';
    ...
    if (e.type === 'user') {
      ...
      if (tur && typeof tur === 'object') {
        if (tur.agentId && tuid) {
          if (pendingCause !== 'compact') pendingCause = 'subagent';
    ...
    if (!prev) {
      // Streamed chunks of one request share the key: the context of the call is fixed at its first chunk.
      const ts = lastTs ?? 0;
      calls.set(key, {
        key,
        ts,
        ...
        agentId,
        gapMs: prevCallTs === null ? null : ts - prevCallTs,
        after: pendingCause,
      });
      pendingCause = null;
      prevCallTs = ts;
    } else if (usage.output >= prev.usage.output) {
      calls.set(key, { ...prev, usage, effort: typeof e.effort === 'string' ? e.effort : prev.effort });
    }
  }
...
/**
 * Why a call had to rebuild the cache, which decides what could have prevented it:
 *  - `idle`     the gap since the previous call outlived the cache TTL (a pause): prevented by
 *               warming, or by starting fresh with a handoff instead of resuming;
 *  - `subagent` the thread waited on a subagent longer than the TTL: prevented by warming while
 *               it runs, or by shorter delegated batches;
 *  - `compact`  a compaction rewrote the history: expected, it is a new prefix;
 *  - `start`    first call of the transcript;
 *  - `other`    the prefix changed with the cache still alive (model switch, edited instructions).
 * @param {{gapMs?: number|null, after?: string|null, usage: ReturnType<typeof toUsage>}} call
 */
export function breakCause(call) {
  if (call.after === 'compact') return 'compact';
  if (call.gapMs === null || call.gapMs === undefined) return 'start';
  if (call.gapMs >= ttlMsOf(call.usage)) return call.after === 'subagent' ? 'subagent' : 'idle';
  return 'other';
}

export const BREAK_CAUSES = ['idle', 'subagent', 'compact', 'start', 'other'];

/**
 * Cache rebuilds grouped by cause, with what they cost. `idle` + `subagent` is the ceiling of what
 * keeping the cache warm could have saved (before paying for the warming itself).
 * @param {{cause: string, uncached: number, usd: number|null, ttl: string}[]} breaks
 */
export function summarizeBreaks(breaks) {
  /** @type {Record<string, {count: number, tokens: number, usd: number}>} */
  const byCause = Object.fromEntries(BREAK_CAUSES.map((c) => [c, { count: 0, tokens: 0, usd: 0 }]));
  const ttl = { '5m': 0, '1h': 0 };
  for (const b of breaks) {
    const g = byCause[b.cause] || byCause.other;
    g.count++;
    g.tokens += b.uncached;
    g.usd += b.usd || 0;
    ttl[b.ttl === '1h' ? '1h' : '5m']++;
  }
  const avoidable = byCause.idle.usd + byCause.subagent.usd;
  return { byCause, ttl, avoidableUsd: avoidable };
}

/**
 * Lines for `stats` and `trend`.
 * @param {ReturnType<typeof summarizeBreaks>} s
 * @param {number} totalUsd
 * @param {(n: number) => string} fmtTok
 */
export function formatBreaks(s, totalUsd, fmtTok) {
  const usd = (n) => `$${n.toFixed(2)}`;
  const pct = (n) => (totalUsd > 0 ? ` (${((100 * n) / totalUsd).toFixed(1)}% of cost)` : '');
  const parts = BREAK_CAUSES.filter((c) => s.byCause[c].count)
    .map((c) => `${c} ${s.byCause[c].count} · ${fmtTok(s.byCause[c].tokens)} · ${usd(s.byCause[c].usd)}`);
  return [
    `cache rebuilds by cause: ${parts.join('  |  ') || 'none'}`,
    `cache TTL written: 5m ×${s.ttl['5m']} · 1h ×${s.ttl['1h']}  ·  avoidable by warming (idle + subagent): ${usd(s.avoidableUsd)}${pct(s.avoidableUsd)}`,
  ];
}
...
export function parseSession(ref, opts = {}) {
  ...
  const ingest = (call, agentType) => {
    ...
    if (uncached > threshold) {
      const written = { ...emptyUsage(), cacheWrite5m: call.usage.cacheWrite5m, cacheWrite1h: call.usage.cacheWrite1h, input: call.usage.input };
      cacheBreaks.push({
        ts: call.ts,
        uncached,
        total: totalInput(call.usage),
  ...
  const mapFinish = (obj) => Object.fromEntries(Object.entries(obj).map(([k, v]) => [k, finishBucket(v)]));
  const usage = finishBucket(total);
  return {
    ...
    usage,
    main: finishBucket(mainBucket),
    subagents: finishBucket(subBucket),
    ...
    cacheBreaks,
    // Claude Code picks the TTL per bucket, and `promptCacheTtl` is the main conversation's.
    ttlWhatIf: hourTtlWhatIf(main.calls),
    unknownModels: usage.unknownModels,
  };
}

/**
 * The verdict on a one-hour cache for the main conversation, as a line for `stats` and `trend`, or
 * null when there is nothing to decide (the account already writes with the hour, or no 5m writes).
 * The cut-off is ours, fixed: under a 1% difference it is not worth a settings change.
 * @param {import('../../core/metrics/cache-ttl.mjs').TtlWhatIf} w
 * @returns {string|null}
 */
export function formatTtlWhatIf(w) {
  if (!w.writes5m || w.actualUsd <= 0) return null;
  const pct = (100 * (w.hourUsd - w.actualUsd)) / w.actualUsd;
  const head = `cache TTL 1h what-if (main thread): $${w.actualUsd.toFixed(2)} → $${w.hourUsd.toFixed(2)} (${pct > 0 ? '+' : ''}${pct.toFixed(1)}%)`;
  const rebuilds = w.rebuildsAvoided ? `, ${w.rebuildsAvoided} rebuild${w.rebuildsAvoided === 1 ? '' : 's'} would have been reads` : '';
  if (pct <= -1) return `${head}${rebuilds} · worth it: set "promptCacheTtl": "1h" in ~/.claude/settings.json (Claude Code ≥ 2.1.242)`;
  if (pct >= 0) return `${head} · not worth it here: keep the 5-minute cache`;
  return `${head}${rebuilds} · about the same: no need to change`;
}
